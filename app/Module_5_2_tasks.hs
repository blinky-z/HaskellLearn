...
-- кроме того, что мы с помощью функции f вычисляем какое-то значение типа b, мы также вместе с этим значением добавляем
-- комментарий для записи в лог
...
-- при этом реализация должна быть такова, что мы сохраняем прошлый контекст, то есть все старые записи лога сохраняются
...
-- идея такая: начинаем идти сначала и применять оператор (>>=) по очереди к результату слева, справа же находятся функции
-- функция будет доставать значение и контекст с операнда слева и строить новый контейнер
-- инициализирующий элемент - Log с пустыми сообщениями и значением x, которое было изначально передано в функцию