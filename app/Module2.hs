...
-- это одно и то же
--
-- также следует поднять вопрос, почему при передаче 3 аргументов (const id 2 5), не произошло ошибки,
-- ведь функция const требует всего 2
-- дело в том, что все функции в хаскеле - curried
-- функции в хаскеле не принимают много аргументов, они принимают всего один аргумент, и возвращают функция одного аргумента
