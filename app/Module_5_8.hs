...
-- итак, выводы по монадам Reader, Writer, State:
-- в Reader мы могли работать только с каким-то константным окружением, в State же мы можем изменять состояние (окружение).
-- Это и есть обобщение идей Reader и Writer: в состояние пишем, из состояния читаем.
-- Передача состояния (окружение в Reader) - унаследована от Reader,
-- возможность изменять состояние (лог в Writer) - из Writer.
-- Да, в Writer не передается состояние, оно уже хранится внутри монады как пара (значение, лог), однако как раз так и тут
-- смешение идей Reader и Writer - окружения и лога.