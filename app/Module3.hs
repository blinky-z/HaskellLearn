...
change c | c < 0 = []
         | c == 0 = [[]]
         | otherwise = [x:xs | x <- [2,3,7] {-coins-}, xs <- change $ c - x]
...
-- только элементы списка, то первый операнд имеет тип b, т.е. результат вычисления сворачивающей функции, которая вовзращает
